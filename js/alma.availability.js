...
Drupal.almaAvailability.get_details = function (callback) {
  ...
  if (Drupal.almaAvailability.id_list.length > 0) {
    $.getJSON(Drupal.settings.alma.basePath + 'alma/item/' + Drupal.almaAvailability.id_list.join(',') + '/details', {}, callback);
  }
};
...
Drupal.almaAvailability.get_availability = function (callback) {
  ...
  if (Drupal.almaAvailability.id_list.length > 0) {
    $.getJSON(Drupal.settings.alma.basePath + 'alma/item/' + Drupal.almaAvailability.id_list.join(',') + '/availability', {}, callback);
  }
};
